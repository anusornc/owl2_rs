...
impl OWLParser {
    ...
    pub fn parse_entity(input: &str) -> Result<Entity, Box<pest::error::Error<Rule>>> {
        ...
        let entity = match inner_rule_pair.as_rule() {
            Rule::class => {
                ...
                Entity::Class(Class(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::datatype => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                Entity::Datatype(Datatype(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::object_property => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                Entity::ObjectProperty(ObjectProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::data_property => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                Entity::DataProperty(DataProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::annotation_property => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                Entity::AnnotationProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?)
            },
            Rule::named_individual => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                Entity::NamedIndividual(OWLParser::parse_iri(iri_str).map_err(Box::new)?)
            },
            _ => unreachable!(),
        };
        Ok(entity)
        ...
        if let Some(next_pair) = inner_pairs.next() {
            match next_pair.as_rule() {
                Rule::iri => {
                    ...
                    datatype = Datatype(OWLParser::parse_iri(next_pair.as_str()).map_err(Box::new)?);
                }
                Rule::lang_tag => {
    ...
    pub fn parse_class_expression(input: &str) -> Result<ClassExpression, Box<pest::error::Error<Rule>>> {
        ...
        let class_expression = match inner_rule_pair.as_rule() {
            Rule::class => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                ClassExpression::Class(Class(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::object_intersection_of => {
                let classes: Vec<ClassExpression> = inner_rule_pair.into_inner().map(|p| OWLParser::parse_class_expression(p.as_str()).map_err(Box::new)).collect::<Result<Vec<_>, _>>()?;
    ...
    pub fn parse_object_property(input: &str) -> Result<ObjectProperty, Box<pest::error::Error<Rule>>> {
        ...
        Ok(ObjectProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
    }
    ...
    pub fn parse_object_property_expression(input: &str) -> Result<ObjectPropertyExpression, Box<pest::error::Error<Rule>>> {
        ...
        let object_property_expression = match inner_rule_pair.as_rule() {
            Rule::object_property => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().as_str();
                ObjectPropertyExpression::ObjectProperty(ObjectProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::object_inverse_of_rule => {
                let iri_str = inner_rule_pair.into_inner().next().unwrap().into_inner().next().unwrap().as_str();
                ObjectPropertyExpression::InverseObjectProperty(ObjectProperty(OWLParser::parse_iri(iri_str).map_err(Box::new)?))
            },
            Rule::object_property_chain => {
                let properties: Vec<ObjectPropertyExpression> = inner_rule_pair.into_inner().map(|p| OWLParser::parse_object_property_expression(p.as_str()).map_err(Box::new)).collect::<Result<Vec<_>, _>>()?;
    ...
    pub fn parse_class_axiom(input: &str) -> Result<ClassAxiom, Box<pest::error::Error<Rule>>> {
        ...
        let class_axiom = match inner_rule_pair.as_rule() {
            Rule::sub_class_of => {
                let mut inner = inner_rule_pair.into_inner();
                let sub_class = OWLParser::parse_class_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                let super_class = OWLParser::parse_class_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
            ...
            Rule::disjoint_union => {
                let mut inner = inner_rule_pair.into_inner();
                let class_expr = OWLParser::parse_class_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                let class = if let ClassExpression::Class(c) = class_expr {
                    c
                } else {
                    panic!("Expected a Class in DisjointUnion, but got {:?}", class_expr);
                };
                let disjoint_classes: Vec<ClassExpression> = inner.map(|p| OWLParser::parse_class_expression(p.as_str()).map_err(Box::new)).collect::<Result<Vec<_>, _>>()?;
    ...
    pub fn parse_object_property_axiom(input: &str) -> Result<ObjectPropertyAxiom, Box<pest::error::Error<Rule>>> {
        ...
        let object_property_axiom = match inner_rule_pair.as_rule() {
            Rule::sub_object_property_of => {
                let mut inner = inner_rule_pair.into_inner();
                let sub_property = OWLParser::parse_object_property_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                let super_property = OWLParser::parse_object_property_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
            ...
            Rule::inverse_object_properties => {
                let mut inner = inner_rule_pair.into_inner();
                let prop1 = OWLParser::parse_object_property_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                let prop2 = OWLParser::parse_object_property_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
            ...
            Rule::object_property_domain => {
                ...
                let domain = OWLParser::parse_class_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                ObjectPropertyAxiom::ObjectPropertyDomain { property, domain }
            },
            Rule::object_property_range => {
                let mut inner = inner_rule_pair.into_inner();
                let property = OWLParser::parse_object_property_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                let range = OWLParser::parse_class_expression(inner.next().unwrap().as_str()).map_err(Box::new)?;
                ObjectPropertyAxiom::ObjectPropertyRange { property, range }
            },
            Rule::functional_object_property => {
                let property = OWLParser::parse_object_property_expression(inner_rule_pair.into_inner().next().unwrap().as_str()).map_err(Box::new)?;
    ...
    pub fn parse_ontology(input: &str) -> Result<crate::Ontology, Box<pest::error::Error<Rule>>> {
        let mut pairs = OWLParser::parse(Rule::ontology, input).map_err(Box::new)?;
